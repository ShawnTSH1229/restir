...
Texture2D<float4> history_reservoir_ray_radiance : register(t1);//xyz only
Texture2D<float>  history_reservoir_hit_distance : register(t2);
Texture2D<float4> history_reservoir_hit_normal : register(t3);//xyz only
Texture2D<float4> history_reservoir_weights : register(t4);//xyz only

Texture2D<float4> history_downsampled_world_pos : register(t5);//xyz only
Texture2D<float4> history_downsampled_world_normal : register(t6);//xyz only

Texture2D<float4> reservoir_ray_direction : register(t7);
Texture2D<float4> reservoir_ray_radiance : register(t8);//xyz only
Texture2D<float>  reservoir_hit_distance : register(t9);
Texture2D<float4> reservoir_hit_normal : register(t10);//xyz only
Texture2D<float4> reservoir_weights : register(t11);//xyz only

Texture2D<float4> downsampled_world_pos : register(t12);//xyz only
Texture2D<float4> downsampled_world_normal : register(t13);//xyz only
