...
{
	const float Sign = TangentZ.z >= 0 ? 1 : -1; // Sign = -1
	const float a = -rcp( Sign + TangentZ.z ); //a = 1
	const float b = TangentZ.x * TangentZ.y * a; //b = 0
	
	float3 TangentX = { 1 + Sign * a * Pow2( TangentZ.x ), Sign * b, -Sign * TangentZ.x }; //0, 0, 0
	float3 TangentY = { b,  Sign + a * Pow2( TangentZ.y ), -TangentZ.y }; // 0, 0, -1

	return float3x3( TangentX, TangentY, TangentZ );// 0 1 0
}
...
{
    //if(HitKind() == HIT_KIND_TRIANGLE_FRONT_FACE) // Fix me. There are a lot of two side mesh in sponza scene.
    {
