...
void RestirApp::RenderScene( void )
{
    ...
    {
        float GILightOrientation = Sponza::m_SunOrientation;
        float GISunInclination = 0.25;

        float costheta = cosf(GILightOrientation);
        float sintheta = sinf(GILightOrientation);
        float cosphi = cosf(GISunInclination * 3.14159f * 0.5f);
        float sinphi = sinf(GISunInclination * 3.14159f * 0.5f);

        Vector3 SunDirection = Normalize(Vector3(costheta * cosphi, sinphi, sintheta * cosphi));
        Vector3 ShadowBounds = Vector3(m_ModelInst.GetRadius());
        
        m_SunShadowCamera.UpdateMatrix(-SunDirection, Vector3(0, -500.0f, 0), Vector3(5000, 3000, 3000), (uint32_t)g_ShadowBuffer.GetWidth(), (uint32_t)g_ShadowBuffer.GetHeight(), 16);

        {
            ...
            GetGlobalResource().restirSRayTracingInfo.restirTextureSize = DirectX::XMFLOAT4(g_ReservoirRayDirectionHistoryPingPong[0].GetWidth(), g_ReservoirRayDirectionHistoryPingPong[0].GetHeight(), 1.0 / g_ReservoirRayDirectionHistoryPingPong[0].GetWidth(), 1.0 / g_ReservoirRayDirectionHistoryPingPong[0].GetHeight());

            GetGlobalResource().restirSceneInfo.PreViewProjMatrix = pre_view_matrix;
            GetGlobalResource().restirSceneInfo.g_restir_texturesize = DirectX::XMFLOAT2(g_ReservoirRayDirectionHistoryPingPong[0].GetWidth(), g_ReservoirRayDirectionHistoryPingPong[0].GetHeight());
        ...
        restirResamplingPass.RestirResampling(gfxContext);
        integrateAndDenoise.IntegrateAndDenoise(gfxContext);
    ...
    GetGlobalResource().incrementFrameIndex();
}
